...
| Shift indicator cable unhooked at column (per seller) | Confirm not the neutral-safety interlock — test "foot on brake, try to crank in Drive/Reverse" | Open — `330 neutralisation du démarreur` passed inspection; access procedure in [`shift-indicator-cable.md`](shift-indicator-cable.md) | — |
