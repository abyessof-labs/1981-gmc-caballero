...
| Front seats are 1982 Firebird buckets **bolted to wood**, sitting slumped — not a valid anchorage. Factory seat was a **split bench, fold-down** per the previous owner, so the inner floor mounts are already present and no bracket set or welding is needed — only G-body tracks | **Safety — inspection-critical** | Open — see [`seats-sourcing.md`](seats-sourcing.md) |
