...
**This decision is tied to the bezel.** Per [`air-system.md`](air-system.md), the centre bezel on this
car is already broken *and* cut open for a non-factory radio, and no one reproduces it — so a
factory-shaped block-off plate will not fill the opening you actually have. Since a replacement bezel
has to be sourced regardless, deleting the radio makes that job easier, not harder: shop for an
**unmolested 81–83 bezel with the original GM opening** and blank it with a factory-style delete
plate, rather than a pre-cut DIN bezel that then needs a DIN-sized filler. Settle the bezel and the
blanking plate as one purchase.
